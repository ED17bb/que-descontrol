...
const triggerFeedback = (type, audioEnabled = true) => {
  if (audioEnabled) {
    try {
...
const EVENTOS_DB = {
  PELIGRO: [
    {
      text: '¡TERREMOTO! Todos cambian de asiento hacia la izquierda.',
      penalty: 0,
      timer: 10,
    },
    {
      text: 'Haz 10 flexiones o retrocede 3 casillas.',
      penalty: -3,
      actionText: 'Si fallas: -3',
      timer: 20,
    },
    {
      text: 'Mantén una sentadilla isométrica hasta tu próximo turno.',
      penalty: -2,
      actionText: 'Si caes: -2',
    },
    {
      text: 'El suelo es lava: Súbete a una silla YA.',
      penalty: -5,
      actionText: 'El último: -5',
      timer: 5,
    },
    {
      text: 'Haz el puente (yoga) por 10 segundos.',
      penalty: -2,
      actionText: 'Falla y retrocede 2',
      timer: 15,
    },
    {
      text: 'Camina como cangrejo alrededor de la mesa.',
      penalty: 0,
      timer: 20,
    },
    {
      text: 'Debes jugar con los ojos cerrados hasta tu próximo turno.',
      penalty: 0,
    },
  ],
  TRIVIA: [
    ...
    {
      text: "¿Quién canta 'Thriller'?",
      answer: 'Michael Jackson',
      bonus: 2,
      actionText: 'Bonus +2 cantando',
      timer: 15,
    },
    {
      ...
      answer: 'Tres',
      bonus: 2,
      actionText: 'Acierta y avanza 2',
      timer: 10,
    },
    {
      text: '¿Qué ingrediente NO lleva la pizza Hawaiana?',
      answer: 'El buen gusto (es broma, Piña)',
      penalty: 0,
    },
    {
      text: "Completa: 'Camarón que se duerme...'",
  ...
  CHAMUYO: [
    {
      text: 'Envía un audio cantando a tu ex (o a tu madre).',
      penalty: -5,
      actionText: 'Hazlo o -5 casillas',
      timer: 30,
    },
    {
      text: 'Deja que el grupo lea tu último WhatsApp.',
      penalty: -3,
      actionText: 'Hazlo o -3 casillas',
      timer: 15,
    },
    {
      text: 'Imita a otro jugador. Si adivinan quién es, avanzas.',
      bonus: 2,
      actionText: 'Avanza 2 si adivinan',
      timer: 20,
    },
    {
      text: 'Llama a una pizzería y pide una hamburguesa.',
      penalty: -4,
      actionText: 'Hazlo o -4 casillas',
      timer: 45,
    },
    {
      text: 'Debes hablar con acento extranjero hasta tu próximo turno.',
      penalty: 0,
    },
    {
      text: 'Elige a alguien para que te haga cosquillas por 10 seg.',
      penalty: -2,
      actionText: 'Aguanta o -2',
      timer: 15,
    },
  ],
  SUERTE: [
    {
      text: '¡Un taxi te lleva! Avanza rápido.',
      bonus: 3,
      actionText: '¡Avanzas 3!',
    },
    {
      text: 'Olvidaste la billetera. Vuelve a buscarla.',
      penalty: -2,
      actionText: 'Retrocedes 2',
    },
    {
      text: 'Te encontraste dinero.',
      bonus: 1,
      actionText: 'Avanzas 1 casilla',
    },
    {
      text: 'Atajo secreto desbloqueado.',
      bonus: 5,
      actionText: '¡MEGA SALTO +5!',
    },
    {
      text: 'Pinchaste rueda. Pierdes un turno.',
      penalty: 0,
      actionText: 'Pasa el dado',
    },
    {
      text: 'Intercambia lugar con el jugador que va último.',
      penalty: 0,
      actionText: '¡Cambio de lugares!',
    },
  ],
  VS: [
    {
      text: 'Piedra, Papel o Tijera con el de tu derecha.',
      penalty: -2,
      actionText: 'El perdedor retrocede 2',
    },
    {
      text: 'Duelo de miradas con el de la izquierda.',
      penalty: 0,
      actionText: 'El que parpadea pierde',
    },
    {
      text: 'Pulseada china (pulgares) con quien elijas.',
      bonus: 2,
      actionText: 'Ganador avanza 2',
    },
    {
      text: 'Todos votan: ¿Quién es el más chamuyero?',
      penalty: -3,
      actionText: 'El elegido retrocede 3',
    },
  ],
};
  ...
  const particles = useMemo(
    () =>
      [...Array(50)].map((_, i) => ({
        ...
        bg: ['#FFD700', '#FF6347', '#32CD32', '#1E90FF'][
...
// --- CÁMARA DEL GANADOR ---
const WinnerCamera = ({ onCapture, audioEnabled }) => {
  ...
  return (
    <div className="relative w-full max-w-xs mx-auto rounded-2xl overflow-hidden border-4 border-yellow-400 shadow-2xl mb-4 bg-black">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none">
        <Crown
          size={80}
          className="text-yellow-400 fill-yellow-400 drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] animate-bounce"
        />
      </div>
    </div>
  );
...
const CountdownCircle = ({ seconds, onComplete, audioEnabled }) => {
...
const Dice3D = ({ rolling, value, onRoll, audioEnabled }) => {
  const faces = {
  ...
  const currentRotation = rolling
    ? `rotateX(${Math.random() * 1080 + 720}deg) rotateY(${
        Math.random() * 1080 + 720
      }deg)`
    : faces[value] || faces[1];

  return (
    <div
      className="relative w-28 h-28 perspective-1000 group cursor-pointer"
      onClick={onRoll}
    >
      <div
      ...
      >
        {[
          ...
          {
            id: 6,
            rot: 'rotateY(180deg) translateZ(56px)',
            dots: [0, 2, 3, 5, 6, 8],
          },
          { id: 2, rot: 'rotateX(90deg) translateZ(56px)', dots: [0, 8] },
          {
            id: 5,
            rot: 'rotateX(-90deg) translateZ(56px)',
            dots: [0, 2, 4, 6, 8],
          },
            ...
            {[...Array(9)].map((_, i) => (
              <div
                ...
                className={`rounded-full transition-all ${
                  face.dots.includes(i)
                    ? 'bg-black shadow-inner scale-100'
      ...
      <div
        className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-24 h-6 bg-black/40 blur-lg rounded-full transition-all duration-300"
        style={{
          transform: rolling
            ? 'translateX(-50%) scale(0.6)'
            : 'translateX(-50%) scale(1)',
          opacity: rolling ? 0.4 : 0.6,
        }}
      />
    </div>
  );
...
const Casilla = ({ x, y, index, typeData, isTarget }) => {
  ...
  return (
    ...
    >
      ...
      <span className="relative text-white font-bold text-lg drop-shadow-md z-10">
        {index + 1}
      </span>
    </div>
  );
...
const Ficha = ({ x, y, color, isActive, avatar }) => {
  return (
    <div
      style={{
        position: 'absolute',
        ...
        boxShadow: isActive
          ? `0 0 30px ${color}, 0 10px 20px rgba(0,0,0,0.5)`
...
const EventModal = ({ event, typeData, onApply, audioEnabled }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-slate-400 text-xs font-bold mb-4 uppercase tracking-[0.2em]">
          {typeData.label}
        </p>
        ...
        {timerExpired && (
          <div className="mb-4 text-red-500 font-black text-2xl animate-bounce uppercase">
            ¡TIEMPO!
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 relative z-10">
          {event.penalty || event.bonus ? (
            <>
              ...
              <button
                onClick={() => {
                  triggerFeedback(event.bonus ? 'win' : 'bad', audioEnabled);
                  onApply(true);
                }}
...
export default function App() {
  ...
  const [winnerPhoto, setWinnerPhoto] = useState(null); // Nuevo: Foto del ganador
  ...
  // Guardar en cada cambio relevante
  useEffect(() => {
    if (players.length > 0) {
      const stateToSave = {
        players,
  ...
  // --- GENERACIÓN DEL TABLERO (ESPIRAL INVERTIDA: AFUERA -> ADENTRO) ---
  const tilesData = useMemo(() => {
    const tiles = [];
    let angle = 0;

    // Configuración Base: Empezamos LEJOS (Radio Grande) y vamos al CENTRO (Radio Pequeño)
    let maxRadius = 160;
    let minRadius = 20;

    // Ajustes según cantidad de casillas para que quepan
    if (totalTiles >= 90) maxRadius = 220;
    else if (totalTiles <= 30) maxRadius = 120;

    const angleIncrement = 0.45; // Que tanto gira

    for (let i = 0; i < totalTiles; i++) {
      // Interpolación lineal inversa para el radio:
      // Cuando i=0 (inicio), t=0 -> radius = maxRadius
      // Cuando i=total (fin), t=1 -> radius = minRadius
  ...
  const addPlayer = () => {
    ...
    const availableAvatars = AVATARES.filter(
      (a) => !players.some((p) => p.avatar === a)
    );
    const randomAvatar =
  ...
  const startGame = () => {
    if (players.length > 0) setGameState('playing');
  ...
  const updatePlayerPosition = (newPos) => {
    ...
    if (diff !== 0)
      setLastLog(
        `${newPlayers[turnIndex].name} se movió ${diff > 0 ? '+' : ''}${diff}`
      );
  };

  const triggerEvent = (posIndex) => {
  ...
  const handleEventDecision = (applyConsequence) => {
    ...
    if (applyConsequence && (data.penalty || data.bonus)) {
      ...
      setScreenFlash(
        moveAmount > 0 ? 'bg-green-500/30' : 'bg-red-500/30 animate-shake'
      );
  ...
  const resetGame = () => {
    // Limpiar LocalStorage al reiniciar completamente
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-900 text-white font-sans p-6 overflow-hidden relative">
        <button
          ...
          className="absolute top-4 right-4 p-3 bg-slate-800 rounded-full hover:bg-slate-700 z-50 transition-colors"
        >
          {audioEnabled ? (
        ...
        <p className="text-slate-400 mb-8 text-xl font-medium tracking-widest uppercase">
          Edición Fiesta
        </p>
        ...
        <div className="w-full max-w-md bg-slate-800/80 backdrop-blur-sm p-6 rounded-3xl shadow-2xl border border-white/5 z-10">
          <div className="flex bg-slate-900/50 p-1 rounded-xl mb-6 border border-white/5">
            <button
              ...
              className={`flex-1 py-2 rounded-lg font-bold text-xs sm:text-sm transition-all ${
                totalTiles === 30
          ...
          <div className="flex gap-2 mb-6">
            ...
            <button
              onClick={addPlayer}
              className="bg-blue-600 p-4 rounded-xl hover:bg-blue-500 transition-transform active:scale-95 shadow-lg shadow-blue-500/30"
            >
              <UserPlus size={28} />
            </button>
          </div>
          <div className="space-y-3 mb-8 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
            {players.map((p) => (
              <div
                key={p.id}
                className="flex items-center justify-between bg-slate-700/50 p-3 px-5 rounded-xl border-l-8 transition-all hover:translate-x-1"
                style={{ borderColor: p.color }}
              >
                <span className="font-bold text-lg text-white">{p.name}</span>
                <span className="text-2xl filter drop-shadow-md animate-bounce">
                  {p.avatar}
                </span>
              </div>
            ))}
          ...
          {/* Botón de Borrar Partida Guardada si existe */}
            ...
            players.length === 0 && (
              <div className="mb-4 text-center">
                <button
                  ...
                  className="text-red-400 text-xs flex items-center justify-center gap-1 mx-auto hover:text-red-300"
                >
          ...
          <button
            ...
            className={`w-full py-5 rounded-2xl text-xl font-black flex items-center justify-center gap-3 transition-all ${
              players.length > 0
                ? 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white shadow-lg shadow-orange-500/30 hover:scale-[1.02] hover:shadow-orange-500/50'
  ...
  const backgroundGradient = `radial-gradient(circle at center, 
    rgba(${15 + gameProgress * 60}, ${23 - gameProgress * 20}, ${
    42 - gameProgress * 40
  }, 1) 0%, 
    rgba(${15 + gameProgress * 20}, ${23 - gameProgress * 10}, ${
    42 - gameProgress * 30
  }, 1) 100%)`;

  return (
    <div
      className={`relative w-full h-screen overflow-hidden font-sans select-none text-white transition-colors duration-200 ${
        screenFlash || ''
      }`}
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full p-4 z-40 flex justify-between items-start pointer-events-none">
          ...
          <div>
            ...
            {players[turnIndex] && (
              <h2
                ...
                style={{ color: players[turnIndex].color }}
              >
        ...
        <button
          onClick={() => setAudioEnabled(!audioEnabled)}
          className="pointer-events-auto p-3 bg-slate-800/80 backdrop-blur rounded-full hover:bg-slate-700 border border-white/10 transition-transform active:scale-90"
        >
          {audioEnabled ? (
            <Volume2 size={24} className="text-green-400" />
          ) : (
      ...
      >
        <svg
          className="absolute overflow-visible opacity-40"
      ...
      <div className="absolute bottom-8 left-0 w-full flex flex-col items-center justify-center z-40 pointer-events-none">
        {gameState === 'win' && players[turnIndex] && (
          <div className="mb-6 bg-yellow-400 text-black p-6 rounded-3xl shadow-2xl animate-in slide-in-from-bottom-20 pointer-events-auto text-center border-4 border-black max-w-xs mx-4 relative overflow-hidden">
            ...
            {/* CÁMARA O FOTO DEL GANADOR */}
            {!winnerPhoto ? (
              <div className="mb-4">
                <WinnerCamera
                  onCapture={setWinnerPhoto}
                  audioEnabled={audioEnabled}
                />
              ...
              <div className="relative mb-4">
                <img
                  src={winnerPhoto}
                  alt="Winner"
                  className="w-full h-64 object-cover rounded-2xl border-4 border-black rotate-2"
                />
                <Crown
                  size={64}
                  className="absolute -top-8 -right-8 text-yellow-400 fill-yellow-400 drop-shadow-lg animate-bounce"
                />
              </div>
            )}
            ...
            <p className="text-lg mb-4 font-bold leading-tight">
              {players[turnIndex].name}
            </p>
            <button
              onClick={resetGame}
              className="w-full bg-black text-white px-6 py-3 rounded-xl font-black flex items-center justify-center gap-2 hover:bg-slate-800 transition-colors shadow-lg"
            >
        ...
        {gameState === 'playing' && !currentEvent && (
          <div className="pointer-events-auto mb-4 animate-in slide-in-from-bottom-10 duration-500 relative">
            ...
            {!isRolling && (
              <>
                <p className="text-center text-slate-400 text-xs font-bold uppercase tracking-widest mt-4 opacity-50 animate-pulse">
                  Toca el dado
                </p>
                {lastLog && (
      ...
      <div
        className={`absolute inset-0 pointer-events-none bg-[radial-gradient(transparent_0%,_rgba(0,0,0,${
          0.5 + gameProgress * 0.4
        })_100%)] z-30`}
      />
